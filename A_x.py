...
def Ax_AGGD(O_1):
    O_1_x,O_1_y=torch.gradient(O_1)
    Ax=torch.atan(O_1_x/O_1_y)
    Shifts=torch.tensor([[0,1],[1,0],[1,1],[-1,1]])
    shifted=torch.roll(Ax,shifts=(0,1),dims=(0,1))
    diff=Ax-shifted
    m,n=diff.shape
    ...
    A_x_feat=torch.tensor([alpha,meanparam,leftstd**2,rightstd**2])
    return A_x_feat
