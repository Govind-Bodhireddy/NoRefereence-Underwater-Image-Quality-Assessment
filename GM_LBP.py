...
def GMO(tensor):
    ...
    GM=GM.numpy()
    LBP_feat_GM=torch.tensor(lbp.lbp(GM,1,8,mapping,'nh'))
    GO=torch.atan(im_dy/im_dx)
    GO=torch.round(GO/(torch.pi/4))*(torch.pi/4)
    GO=GO.numpy()
    LBP_feat_GO=torch.tensor(lbp.lbp(GO,1,8,mapping,'nh'))

    return LBP_feat_GM,LBP_feat_GO
